...
export const scenesApi = {
  ...
  copy(id: number, name: string) {
    return request.post<unknown, ApiResponse<{ scene_id: number }>>(`/v1/scenes/${id}/copy`, { name })
  },
  /** 场景执行历史列表 */
  history(id: number, params?: { page?: number; page_size?: number }) {
    return request.get<unknown, ApiResponse<PageResult<SceneHistoryItem>>>(`/v1/scenes/${id}/history`, { params })
  },
  /** 场景执行实时进度 */
